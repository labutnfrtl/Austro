...
def insertar_datos_LDR(idSensor, luminosidad, reintentos=5, espera=1):
    """
    Inserta datos en la tabla LDR.
    ...
    while intento < reintentos:
        try:
            with conectar(timeout=10) as conexion:
                cursor = conexion.cursor()
                cursor.execute('''
                    INSERT INTO LDR (idSensor, luminosidad)
                    VALUES (?, ?)
                ''', (idSensor, luminosidad))
                conexion.commit()
            return  # Éxito
